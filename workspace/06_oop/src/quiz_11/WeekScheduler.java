...
public class WeekScheduler {
	...
	public void run() {
		while(true) {
			...
			sc.nextLine(); //choice 입력 뒤에 남아있는 enter를 제거하기 위한 코드
			switch(choice) {
		...
		for(int i =0; i<week.length;i++) {
			if(weekNames[i].equals(weekName)) {
				if(week[i].getSchedule() != null || !(week[i].getSchedule().isEmpty())) {
					...
					week[i].setSchedule(schedule);
					System.out.println(weekName + "요일의 스케쥴을 삭제하였습니다.");
				}else {
					System.out.println(weekName + "요일은 스케쥴이 없습니다.");
				}
				break;
	...
	public void updateSchedule() {
		...
		for(int i = 0;i<week.length;i++) {
			if(weekNames[i].equals(weekName)) {
				if(week[i].getSchedule() == null || week[i].getSchedule().isEmpty()) {
					System.out.println(weekName + "요일은 스케쥴이 없습니다.");
					System.out.println("새로운 스케쥴을 등록 하시겟습니까?(Y/N)>>>");
					String yesNo = sc.next();
					sc.nextLine();
					...
					if(yesNo.charAt(0)=='Y' || yesNo.charAt(0)== 'y') {
						...
						String schedule = sc.nextLine();
						week[i].setSchedule(schedule);
						System.out.println(weekName+"요일의 스케쥴이수정되었습니다.");
					}else
						System.out.println("스켖ㄹ 수정이취소되었습니다.");
				}break;
			}
	...
	public void searchSchedule() {
		...
		System.out.println("스케쥴을 조회할 요일(일~토)입력 >>>");//최
		String weekName = sc.next();
		weekName = weekName.substring(0,1);//
		for(int i = 0;i<week.length;i++) {
			if(weekNames[i].equals(weekName)) {
